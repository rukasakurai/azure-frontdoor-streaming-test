...
Streaming is the primary question, but the same deployment is reused to probe Front Door **caching** behaviour. `log-test.sh`, `cache-test.sh`, and `auth-cache-test.sh` (also at the root) read the Front Door access log from Log Analytics; only `log-test.sh` runs in CI, because the other two need a live deployment and are experiments rather than regression checks. See [docs/auth-cache-result.md](../docs/auth-cache-result.md) for the cache experiment write-up.
